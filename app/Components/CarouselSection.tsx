...
const CarouselSection = () => {
  ...
  return (
    <section className='flex flex-col items-center mb-40 '>
      <h2 className='text-lg sm:text-xl font-semibold mt-10 mb-8'>Produtos que você precisa conhecer:</h2>
      <Carousel
          ...
          {CaroulselItens.map((item, index) => (
            <CarouselItem key={index} className='basis-2/3 sm:basis-1/2  '>
              <div className=" bg-primary-foreground rounded">
                ...
                <div className='p-4'>
                  <h3 className='text-base sm:text-lg font-semibold mb-4'>{item.title}</h3>
                  <p className='mb-10'>{item.description}</p>
                  <a href={item.LinkURL} className=' underline font-semibold'>{item.LinkText}</a>
                </div>
              </div>
            </CarouselItem>
          ))}
